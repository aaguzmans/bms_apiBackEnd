...
const getItems = async (req, res) => {
  try {
    ...
    const { docs, pages, total } = await genderModel.paginate({
...
const getItem = async (req, res) => {
  try {
    ...
    const data = await genderModel.findOne({
      where: {
        id
      }
    });
...
const createItem = async (req, res) => {
  try {
    ...
    const data = await genderModel.create(body);
    res.send({ data, user });
  } catch (error) {
...
const updateItem = async (req, res) => {
  try {
    ...
    // const data = await genderModel.findOneAndUpdate( id, body );
    // res.send({ data });

    // Find the record by its primary key (id)
    const existingItem = await genderModel.findByPk(id);
