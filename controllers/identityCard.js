...
const { identityCardModel } = require("../models");
...
const getItems = async (req, res) => {
  try {
    ...
    const { docs, pages, total } = await identityCardModel.paginate({
      where: {
        companyId: companyId
      },
...
const getItem = async (req, res) => {
  try {
    ...
    const data = await identityCardModel.findOne({
      where: {
        id,
        companyId: companyId
      }
...
const createItem = async (req, res) => {
  try {
    ...
    const data = await identityCardModel.create(body);
    res.send({ data, user });
  } catch (error) {
...
const updateItem = async (req, res) => {
  try {
    ...
    // const data = await identityCardModel.findOneAndUpdate( id, body );
    // res.send({ data });

    // Find the record by its primary key (id)
    const existingItem = await identityCardModel.findByPk(id);
