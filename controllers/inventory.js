...
const { inventoryModel, companyModel, measurementModel, supplierModel } = require("../models");
...
const getItems = async (req, res) => {
  try {
    ...
    const { docs, pages, total } = await inventoryModel.paginate({
      where: {
        companyId: companyId
      },
      ...
      include: [
        {
          ...
          as: 'measurement',
          attributes: ['id','measurementName'],
        },
        {
          model: supplierModel, // Suponiendo que tienes un modelo llamado "Service" configurado
          as: 'supplier', // El nombre de la relación en el modelo "PatientCase"
          attributes: ['id', 'supplierName'], // Define los atributos del servicio que deseas incluir
        },
        {
