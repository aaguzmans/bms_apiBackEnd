...
const { serviceinventory } = require("../models");
...
const getItems = async (req, res) => {
  try {
    ...
    const data = await serviceinventory.findAll();
...
const getItem = async (req, res) => {
  try {
    req = matchedData(req);
    const { id } = req;
    const data = await serviceinventory.findByPk(id);
...
const createItem = async (req, res) => {
  try {
    ...
    const data = await serviceinventory.create(body);
    res.send({ data });
  } catch (error) {
...
const updateItem = async (req, res) => {
  try {
    ...
    const existingItem = await serviceinventory.findByPk(id);
