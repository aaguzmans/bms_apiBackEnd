...
const getItems = async (req, res) => {
  try {
    ...
    // Asegúrate de que per_page sea al menos 8
    const pageSize = Math.max(parseInt(per_page), 8);

    const { docs, pages, total } = await serviceModel.paginate({
      where: {
        companyId: companyId
      },
...
const updateItem = async (req, res) => {
  try {
    ...
    // const data = await service.findOneAndUpdate( id, body );
    // res.send({ data });

    // Find the record by its primary key (id)
    const existingItem = await serviceModel.findByPk(id);
