...
const { supplierModel, companyModel } = require("../models");
...
const getItems = async (req, res) => {
  try {
    ...
    const { docs, pages, total } = await supplierModel.paginate({
      where: {
        companyId: companyId
      },
...
const updateItem = async (req, res) => {
  try {
    ...
    // const data = await supplierModel.findOneAndUpdate( id, body );
    // res.send({ data });

    // Find the record by its primary key (id)
    const existingItem = await supplierModel.findByPk(id);
