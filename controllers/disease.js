...
const { diseaseModel, companyModel } = require("../models");
...
const getItems = async (req, res) => {
  try {
    ...
    const { docs, pages, total } = await diseaseModel.paginate({
      where: {
        companyId: companyId
      },
...
const getItem = async (req, res) => {
  try {
    ...
    const data = await diseaseModel.findOne({
      where: {
        id,
        companyId: companyId
      }
...
const createItem = async (req, res) => {
  try {
    ...
    const data = await diseaseModel.create(body);
    res.send({ data, user });
  } catch (error) {
...
const updateItem = async (req, res) => {
  try {
    ...
    // const data = await diseaseModel.findOneAndUpdate( id, body );
    // res.send({ data });

    // Find the record by its primary key (id)
    const existingItem = await diseaseModel.findByPk(id);
