...
const getItems = async (req, res) => {
  try {
    ...
    const data = await patientHistory.findAll();
...
const getItem = async (req, res) => {
  try {
    req = matchedData(req);
    const { id } = req;
    const data = await patientHistory.findByPk(id);
...
const createItem = async (req, res) => {
  try {
    ...
    const data = await patientHistory.create(body);
    res.send({ data });
  } catch (error) {
...
const updateItem = async (req, res) => {
  try {
    ...
    const existingItem = await patientHistory.findByPk(id);
