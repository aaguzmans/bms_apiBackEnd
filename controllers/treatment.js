...
const { treatmentModel, companyModel } = require("../models");
...
const getItems = async (req, res) => {
  try {
    ...
    const { docs, pages, total } = await treatmentModel.paginate({
      where: {
        companyId: companyId
      },
...
const getItem = async (req, res) => {
  try {
    ...
    const data = await treatmentModel.findOne({
      where: {
        id,
        companyId: companyId
      }
...
const createItem = async (req, res) => {
  try {
    ...
    const data = await treatmentModel.create(body);
    res.send({ data, user });
  } catch (error) {
...
const updateItem = async (req, res) => {
  try {
    ...
    // const data = await treatmentModel.findOneAndUpdate( id, body );
    // res.send({ data });

    // Find the record by its primary key (id)
    const existingItem = await treatmentModel.findByPk(id);
